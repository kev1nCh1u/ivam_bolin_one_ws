class test_package : public Move_Robot
{
public:
	test_package(char *dev_name, int Baudrate);
	~test_package();
	...
	void qrcodeCallback(const move_robot::qrcode &msg);
	void commandCallback(const move_robot::setmap_ctr &command);
...
test_package::test_package(char *dev_name, int Baudrate) : Move_Robot(dev_name, Baudrate)
{
	...
	Rev_onewheel_th = 0;
	Rev_odom_onewheel_th = 0;
	Rev_odom_onewheel_w = 0;
	Rev_odom_onewheel_v = 0;
	Rev_type = 0;
	...
	Clear_ObsModeSubscriber_ = node_.subscribe("Clear_ObsMode", 10, &test_package::ClearCallback, this);
	laserSubscriber_ = node_.subscribe("scan", 10, &test_package::laserCallback, this);
	JoysickSubscriber_ = node_.subscribe("joystick", 5, &test_package::joystickCallback, this);
	...
	control_timer = node_.createTimer(ros::Duration(time_sample), &test_package::timerCallback, this);
}
test_package::~test_package()
{
}

void test_package::commandCallback(const move_robot::setmap_ctr &command)
{
	std::cout << "command.type======" << command.type << std::endl;
	...
	{
		JOYSTICK_SCALAR = remember_JOYSTICK_SCALAR;
	}
}

void test_package::TriggerCallback_all(const std_msgs::Int8 &msg)
{
...
void test_package::timerCallback(const ros::TimerEvent &event)
{
	...
	if (isReveice_joystick && !isReveice_qrcode)
	{
		v_buf = 0.04;
		std::cout << "slam_pose_ = " << slam_pose_.x() << " " << slam_pose_.y() << " " << slam_pose_.z() << std::endl;
		switch (btn_id)
...
void test_package::State_Machine(int &state)
{
	int type, id, previous_type, first_type;
	std::vector<unsigned char> command;
	switch (state)
	{
	...
	case P_STATE_MOVE:
		...
		//std::cout<<"====================================A_misson.size() = "<<A_misson.size()<<std::endl;
		//std::cout<<"====================================now_A_misson = "<<now_A_misson<<std::endl;
		//std::cout<<"====================================now_path_index = "<<now_path_index<<std::endl;
		if (now_path_index > 0)
		{
			...
			else if (previous_type == MISSON_turn_left || previous_type == MISSON_turn_right || previous_type == MISSON_turn_back_left || previous_type == MISSON_turn_back_right || previous_type == MISSON_turn_angle)
			{
				int test_id = A_misson[now_A_misson].sub_missonPath[now_path_index - 1].end;
				for (int i = 0; i < A_misson[now_A_misson].ALL_pathnode.size(); i++)
				{
					if (A_misson[now_A_misson].ALL_pathnode[i].id == test_id)
					{
		...
		else if (now_path_index == 0 && now_A_misson == 0)
		{
			first_type = A_misson[now_A_misson].sub_missonPath[now_path_index].start_type;
			std::cout << "first_type = " << first_type << std::endl;
	...
	case P_STATE_AVOID_DEADLOCK:
		...
		break;
	case P_STATE_QRCODE: //LOADING TIME
		ROS_INFO("qrcode type!!!\n");
		if (qrcode_tag != 0)
		{
			if (qrcode_send)
			{
	...
	case P_STATE_BACK_LEFT:
		ROS_INFO("turn back left!!!\n");
		if (qrcode_send)
		{
		...
		if (Rev_type == 1)
		{
			ROS_INFO("turn back left Success!!!\n");
			p_state_ = P_STATE_TIME_DELAY;
			Rev_type = 0;
			qrcode_send = true;
		}
	...
	case P_STATE_BACK_RIGHT:
		ROS_INFO("turn back right!!!\n");
		if (qrcode_send)
		{
		...
		if (Rev_type == 1)
		{
			ROS_INFO("turn back right Success!!!\n");
			p_state_ = P_STATE_TIME_DELAY;
			Rev_type = 0;
			qrcode_send = true;
		}
	...
	case P_STATE_TURN_ANGLE:
		ROS_INFO("trun angle!!!\n");
		float angular_error = 0;
		static float pre_angular_error = 0;
		float cmd_angular_velocity = 0;
		static bool isFInish = false;
		Caculate_W_rw(turn_angle, slam_pose_, angular_error, pre_angular_error, cmd_angular_velocity, 1);
		//protect
...
bool test_package::Navigation_move(std::vector<SUB_MISSONPATH_SUBPOINT> &myAllSubPath, bool isReSet)
{
	...
	if (!isReSet)
	{
		if (myAllSubPath.size() > 0)
		{
			...
			else
			{
				...
				now_A_misson = ready_path_index; //bug
...
bool test_package::Trajectory_Tracking(int &subpath_index, bool isReSet) //isReSet是fault動作 //true是不動做
{
...
bool test_package::Tracking_Angle_Init(int &subpath_index, bool isReSet)
{
	//return true;
	// if(back_trajectory)
	// {
	...
	if (!isReSet)
	{
		...
		if (subpath_index > 0)
		{
			...
			std::cout << "type = " << type << "  last_type = " << last_type << std::endl;
		...
		if (ready_path_index == 0 && subpath_index == 0)
		{
		...
		if (isInitial)
		{
			...
			//讓所有輪子歸0
			if (fabs(Rev_odom_onewheel_th - 0) <= 0.1)
			{
...
bool test_package::Tracking_Trajectory(int &subpath_index, bool isReSet)
{
	//return true;
	...
	float angular_velocity_kp;
	float angular_velocity_kd;

	angular_velocity_kp = tracking_kp;
	angular_velocity_kd = tracking_kd;
	...
	angular_kp = tracking_kp;
	angular_kd = tracking_kd;
	...
	Eigen::Vector3f back_final_pose;
	...
	if (!isReSet)
	{
		...
		float mission_dis = now_index * 0.04;

		std_msgs::Float32 msg;
		msg.data = mission_dis;
		mission_dis_Publisher_.publish(msg);
		...
		if (now_index < First_tracking)
		{
			...
			{
				//要再正負180內
				std::cout << "ORG TH =  " << atan2((target_pos.y() - robot_pos.y()), (target_pos.x() - robot_pos.x())) - (robot_pos.z()) * 180.0 / M_PI << std::endl;
				std::cout << "ADD ORG TH =  " << angular_velocity_error * 180 / M_PI << std::endl;
				;
				angular_velocity_error = atan2((target_pos.y() - robot_pos.y()), (target_pos.x() - robot_pos.x())) - (robot_pos.z() + Rev_odom_onewheel_th);

				if (fabs(angular_velocity_error) > M_PI)
				{
					if (angular_velocity_error > 0)
						angular_velocity_error = angular_velocity_error - 2 * M_PI;
					else
						angular_velocity_error = angular_velocity_error + 2 * M_PI;
				}

				std::cout << "CH TH =========================  " << angular_velocity_error * 180 / M_PI << std::endl;

				// float dis_temp =  sqrt(1.02*1.02 + target_index_distance*target_index_distance - 2*target_index_distance*1.02*cos(angular_velocity_error));
				//
				// angular_velocity_error = asin(target_index_distance * sin(angular_velocity_error) / dis_temp);
				...
				//     }
				//     else if(fabs(angular_velocity_error) < 0.05)
				//     {
		...
		else
		{
			...
			if(dis_error > pre_dis_error) // kevin dis_error_count
			{
				dis_error_count++;
			}
		...
		if (Endangle)
		{
			...
			// 	{
			// 		  Caculate_W_rw(target_pos.z(), robot_pos, angular_error, pre_angular_error, cmd_angular_velocity, 2);//problem
			...
			// 	else
			// 	{
			// 			Caculate_W_rw(target_pos.z(), robot_pos, angular_error, pre_angular_error, cmd_angular_velocity, 1);//problem
			// 	}
			// 	//protect
			// 	if(fabs(cmd_angular_velocity) >= 0.3){
			...
			//   else if(fabs(cmd_angular_velocity) <= 0.05)
			//   {
			//     if(cmd_angular_velocity > 0) cmd_angular_velocity = 0.05;
			// 		else cmd_angular_velocity = -1*0.05;
			//   }
			// 	// Vx = 0;
			// 	// Vy = 0;
			//   V_rv = 0;
		...
		if (!back_trajectory)
		{
			if (confirm_last_diff_angle && dis_error <= 0.02 && !Endangle)
			{
		...
		else
		{
			...
			if (confirm_last_diff_angle && (dis_error <= 0.05 || dis_error_count > 10) && !Endangle) // kevin Precision
			{
				...
				std::cout << "dis_error_count:"	<< dis_error_count 	<< std::endl;
				dis_error_count = 0;
			}
		}
...
void test_package::laserCallback(const sensor_msgs::LaserScan &scan)
{
...
void test_package::ObsDetect(std::vector<Eigen::Vector3f> &i_obs_container,
							 std::vector<Eigen::Vector3f> &i_obs_region_0,
							 std::vector<Eigen::Vector3f> &i_obs_region_1,
							 std::vector<Eigen::Vector3f> &i_obs_region_2,
							 std::vector<Eigen::Vector3f> &i_obs_region_3,
	...
	for (int i = 0; i < i_obs_container.size(); i++)
	{
		...
		if (p.x() < protect_OBS_Front && p.x() > -1 * Car_Front)
		{
			if (fabs(p.y()) < OBS_SideMiss_Scale * protect_OBS_Side)
			{
				...
				if (p.y() < -1.0 * protect_OBS_Side && p.x() < Car_Front)
					i_obs_region_4.push_back(p);
				else if (p.y() < -1.0 * protect_OBS_Side && p.x() >= Car_Front)
					i_obs_region_3.push_back(p);
				else if (p.x() > -1.0 * Car_Front && fabs(p.y()) <= protect_OBS_Side)
				{
					if (OBS_limilMode || command_OBSMode == 2) //再轉彎時 與 閉障結束回到正常路徑時（diff） 閉障縮短
					{

						if (p.x() < protect_OBS_LimitFront)
					...
					else if (isAvoidObs) // 閉障時會把閉障距離縮短
					{
						...
						if (p.x() < protect_OBS_Front_avoid)
							i_obs_region_2.push_back(p);
				...
				else if (p.y() > protect_OBS_Side && p.x() >= Car_Front)
					i_obs_region_1.push_back(p);
				else if (p.y() > protect_OBS_Side && p.x() < Car_Front)
					i_obs_region_0.push_back(p);
			}
		}
		//避障減速區間
		if (p.x() < (all_obs_dis * 2) && p.x() > -1 * Car_Front)
		{
			if (fabs(p.y()) <= protect_OBS_Side)
				i_obs_region_2_buf.push_back(p);
		}
		//gap觀看區間
		if (p.x() < OBS_LookGap && p.x() > -1 * Car_Front)
		{
			...
			{
				if (p.y() < -1 * protect_OBS_Side)
					i_obs_gap_4.push_back(p);
				else if (fabs(p.y()) <= protect_OBS_Side && p.x() < all_obs_dis * 2.0)
					i_obs_gap_2.push_back(p);
				else if (p.y() > protect_OBS_Side)
					i_obs_gap_0.push_back(p);
			}
...
void test_package::ObsStateMachine(std::vector<Eigen::Vector3f> &i_obs_region_0,
								   std::vector<Eigen::Vector3f> &i_obs_region_1,
								   std::vector<Eigen::Vector3f> &i_obs_region_2,
								   std::vector<Eigen::Vector3f> &i_obs_region_3,
								   std::vector<Eigen::Vector3f> &i_obs_region_4,
...
bool test_package::WaitObsLeave()
{
...
void test_package::RePlanning(int &subpath_index,
							  std::vector<Eigen::Vector3f> &i_obs_gap_0,
							  std::vector<Eigen::Vector3f> &i_obs_gap_2,
							  std::vector<Eigen::Vector3f> &i_obs_gap_4,
							  std::vector<Eigen::Vector3f> &all_point)
{
	...
	if (isInitial_rot)
	{
		...
		if (kinematics_model == "diff")
		{
			...
			isFInish = true;
		if (fabs(Rev_odom_onewheel_v - 0) <= 0.02)
		{
			laser_dead_count += 1;
...
bool test_package::FindKeyPoint(Eigen::Vector3f &target, Eigen::Vector3f &key_p,
								std::vector<Eigen::Vector3f> &i_obs_gap_0,
								std::vector<Eigen::Vector3f> &i_obs_gap_2,
								std::vector<Eigen::Vector3f> &i_obs_gap_4,
								std::vector<Eigen::Vector3f> &all_point)
...
void test_package::CreateNewPath(Eigen::Vector3f &target, Eigen::Vector3f &key_point, int subpath_index, std::vector<Eigen::Vector3f> &new_path)
{
...
bool test_package::AvoidObs()
{
	...
	if (isAvoidObs == true && isFindObs == false)
	{
		...
		float th_err = way_theta - Rev_odom_onewheel_th;
		...
		if (isInitial_move)
		{
			...
			{
				way_theta_stime = Rev_odom_onewheel_th + (th_err / cnt_limit) * time_count_avoid;
				time_count_avoid += 1;
		...
		if (isInitial_move)
		{
			if (fabs(Rev_odom_onewheel_th - W_rw) <= 0.5)
			{
...
bool test_package::WaitDeadlock()
{
...
void test_package::qrcodeCallback(const move_robot::qrcode &msg)
{
	qrcode_x = msg.x;
	qrcode_y = msg.y;
	qrcode_A = msg.A;
	qrcode_tag = msg.Tag;
	//std::cout<<"qr_code"<<std::endl;
	// if(qrcode_tag != 0)
	// {
	//     std::cout<<"qr_code"<<std::endl;
...
void test_package::joystickCallback(const move_robot::joystick &joystick)
{
...
void test_package::Calculate_odom()
{
	//std::cout<<"Rev_V = "<<Rev_odom_onewheel_v<<" "<<" Rev_th =  "<<Rev_onewheel_th<<" Rev_W =  "<<Rev_odom_onewheel_w<<std::endl;
	Rev_odom_onewheel_th = Rev_onewheel_th * M_PI / 180.0;
	NormalizeAngle(Rev_odom_onewheel_th);
	//Rev_odom_onewheel_th = Rev_odom_onewheel_th * -1;//輪子方向
	std::cout << "Rev_V = " << Rev_odom_onewheel_v << " "
			  << " Rev_th =  " << Rev_odom_onewheel_th << " Rev_W =  " << Rev_odom_onewheel_w << " Rev_type = " << Rev_type << std::endl;
	Rev_odom_onewheel_th = 0; //輪子方向
	geometry_msgs::PoseStamped odom_odometry;
	odom_odometry.header.stamp = ros::Time::now();
	odom_odometry.header.frame_id = "odom";
	odom_odometry.pose.position.x = Rev_odom_onewheel_v;
	odom_odometry.pose.position.y = 0;
	odom_odometry.pose.orientation.w = Rev_odom_onewheel_w;
	odom_odometry.pose.orientation.z = 0;
	//std::cout<<"Rev_V = "<<Rev_odom_onewheel_v<<" "<<" Rev_th =  "<<Rev_odom_onewheel_th<<" Rev_W =  "<<Rev_odom_onewheel_w<<std::endl;
	//odometry_Publisher_.publish(odom_odometry);
}

void test_package::RevProcess(double receive_period)
{
	...
	while (1)
	{
		...
		if (mySerial.serial_ok == true)
		{
			...
			readByte = read(mySerial.fd, buff, 16);
			...
			if (rev_buf.size() == 16)
			{
				sendreceive.RevProcess_testWheel_encoder(rev_buf, Rev_odom_onewheel_v, Rev_onewheel_th, Rev_odom_onewheel_w, Rev_type);
				rev_buf.clear();
			}
		}
...
void test_package::RevProcess_Battery(double receive_period)
{
	int Receive_Package_Size = 256;
...
void test_package::Caculate_W_rw(float stop_angle, Eigen::Vector3f robot_pos, float &angular_error, float &pre_angular_error, float &cmd_angular_velocity, int special)
{
	...
	else if (special == 1) //一般模式（進站）
	{
		...
		// float dis_temp =  sqrt(1.02*1.02 + target_index_distance*target_index_distance - 2*target_index_distance*1.02*cos(angular_error));
		//
		// angular_error = asin(target_index_distance * sin(angular_error) / dis_temp);
	...
	else if (special == 2) //特殊模式（導航）
	{
		...
		{
			if (compare_angular_error > 0)
				compare_angular_error = compare_angular_error - 2 * M_PI;
			else
				compare_angular_error = compare_angular_error + 2 * M_PI;
		}
