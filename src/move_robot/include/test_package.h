  ...
  std::vector<unsigned char> command;
	switch (state) {
       ...
       else if(now_path_index == 0 && now_A_misson == 0)
       {
          ...
          if(first_type == MISSON_back_tracking) back_trajectory = true;
          else back_trajectory = false;
       }
    ...
    case P_STATE_TURN_ANGLE:
      ...
      if(fabs(angular_error) <= 0.1) isFInish = true;
      if(isFInish)
      {
          p_state_ = P_STATE_TIME_DELAY;
          pre_angular_error = 0;
...
bool test_package::Tracking_Angle_Init(int &subpath_index, bool isReSet)
{
  ...
  V_rv = 0;
    ...
    if(subpath_index>0)
    {
        ...
        if(type == MISSON_back_tracking && (last_type==MISSON_turn_left || last_type==MISSON_turn_right || last_type==MISSON_turn_back_left || last_type==MISSON_turn_back_right))return true;
    }
    if(ready_path_index == 0 && subpath_index == 0)
    {
        int first_type = A_misson[0].sub_missonPath[0].start_type;
        if(first_type == MISSON_back_tracking)return true;
    }
    ...
    //std::cout<<"AA"<<std::endl;
			...
			if(kinematics_model == "diff")
      {
          ...
          std::cout<<"cmd_angular_velocity "<<cmd_angular_velocity * 180.0/M_PI<<std::endl;
            ...
            if(fabs(cmd_angular_velocity) >= 0.9){
              if(cmd_angular_velocity > 0) cmd_angular_velocity = 0.9;
              else cmd_angular_velocity = -1*0.9;
            }
      ...
      float cmd_v;
        ...
        if(fabs(cmd_angular_velocity) >= 0.7){
          if(cmd_angular_velocity > 0) cmd_angular_velocity = 0.7;
          else cmd_angular_velocity = -1*0.7;
        }
...
std::cout<<"see 5  "<<std::endl;
  ...
  void test_package::qrcodeCallback(const move_robot::qrcode& msg)
  {
	...
	void test_package::Calculate_odom()
	{
      ...
      std::cout<<"Rev_V = "<<Rev_odom_onewheel_v<<" "<<" Rev_th =  "<<Rev_odom_onewheel_th<<" Rev_W =  "<<Rev_odom_onewheel_w<<" Rev_type = "<<Rev_type<<std::endl;
