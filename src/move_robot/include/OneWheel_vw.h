class onewheel_vw: public Move_Robot
{
    public:
      onewheel_vw(char *dev_name, int Baudrate);
      ~onewheel_vw();
      ...
      float Rev_odom_onewheel_w, Rev_odom_onewheel_v;
  		float ini_way_theta;
...
onewheel_vw::onewheel_vw(char *dev_name, int Baudrate):Move_Robot(dev_name, Baudrate)
{
  ...
  qrcodeSubscriber_=node_.subscribe("qrcode", 5, &onewheel_vw::qrcodeCallback, this);
...
void onewheel_vw::qrcodeCallback(const move_robot::qrcode& msg)
{
	qrcode_x = msg.x;
	qrcode_y = msg.y;
	qrcode_A = msg.A;
...
void onewheel_vw::TriggerCallback_all(const std_msgs::Int8& msg)
{
...
void onewheel_vw::timerCallback(const ros::TimerEvent& event)
{
  //std::cout<<"Rev_V = "<<Rev_odom_onewheel_v<<" "<<" Rev_W =  "<<Rev_odom_onewheel_w<<std::endl;
...
void onewheel_vw::ClearCallback(const std_msgs::Int8& msg)
{
...
void onewheel_vw::State_Machine(int& state)
{
int type,id;
	switch (state) {
...
bool onewheel_vw::Navigation_move(std::vector<SUB_MISSONPATH_SUBPOINT> & myAllSubPath, bool isReSet)
{
...
bool onewheel_vw::Trajectory_Tracking(int &subpath_index, bool isReSet)//isReSet是fault動作 //true是不動做
{
...
bool onewheel_vw::Tracking_Angle_Init(int &subpath_index, bool isReSet)
{
  ...
  V_rv = 0;
	...
	if(!isReSet){
		...
		if(isInitial){
			...
			//讓所有輪子歸0
			if(fabs(Rev_odom_onewheel_w - 0) <= 0.1){
...
bool onewheel_vw::Tracking_Trajectory(int &subpath_index, bool isReSet)
{
  std::cout<<"slam_pose_=================="<<slam_pose_.x()<<" "<<slam_pose_.y()<<std::endl;
	static int target_ind = 0;
	static bool isInitial = true;
	static bool isFInish = false;
  ...
  float compare_angular_velocity_error=0; //freeloading 停站使用
    ...
    //std::cout<<"AA"<<std::endl;
      ...
      {
  				//要再正負180內
  				angular_velocity_error = atan2((target_pos.y() - robot_pos.y()) , (target_pos.x() - robot_pos.x())) - (robot_pos.z()+Rev_odom_onewheel_w);
...
// std::cout<<"===============id==========="<<id<<std::endl;
		...
		if(Endangle)
		{
			int id_ = A_misson[ready_path_index].sub_missonPath[subpath_index].end;
				...
				{
					  Caculate_W_rw(target_pos.z(), robot_pos, angular_error, pre_angular_error, cmd_angular_velocity, true);//problem
				...
				{
						Caculate_W_rw(target_pos.z(), robot_pos, angular_error, pre_angular_error, cmd_angular_velocity, false);//problem
				}
				//protect
				if(fabs(cmd_angular_velocity) >= 0.3){
					if(cmd_angular_velocity > 0) cmd_angular_velocity = 0.3;
...
void onewheel_vw::laserCallback(const sensor_msgs::LaserScan& scan)
{
	...
	void onewheel_vw::ObsDetect(std::vector<Eigen::Vector3f> &i_obs_container,
			std::vector<Eigen::Vector3f> &i_obs_region_0,
			std::vector<Eigen::Vector3f> &i_obs_region_1,
			std::vector<Eigen::Vector3f> &i_obs_region_2,
			std::vector<Eigen::Vector3f> &i_obs_region_3,
	...
	void onewheel_vw::ObsStateMachine(std::vector<Eigen::Vector3f> &i_obs_region_0,
			std::vector<Eigen::Vector3f> &i_obs_region_1,
			std::vector<Eigen::Vector3f> &i_obs_region_2,
			std::vector<Eigen::Vector3f> &i_obs_region_3,
			std::vector<Eigen::Vector3f> &i_obs_region_4,
	...
	void onewheel_vw::RePlanning(int &subpath_index,
			std::vector<Eigen::Vector3f> &i_obs_gap_0,
			std::vector<Eigen::Vector3f> &i_obs_gap_2,
			std::vector<Eigen::Vector3f> &i_obs_gap_4,
			std::vector<Eigen::Vector3f> &all_point)
	...
	bool onewheel_vw::FindKeyPoint(Eigen::Vector3f& target, Eigen::Vector3f& key_p,
			std::vector<Eigen::Vector3f> &i_obs_gap_0,
			std::vector<Eigen::Vector3f> &i_obs_gap_2,
			std::vector<Eigen::Vector3f> &i_obs_gap_4,
			std::vector<Eigen::Vector3f> &all_point)
...
std::cout<<"see 5  "<<std::endl;
	...
	void onewheel_vw::CreateNewPath(Eigen::Vector3f& target, Eigen::Vector3f& key_point, int subpath_index, std::vector<Eigen::Vector3f> &new_path)
	{
    ...
    V_rv = 0;
      ...
      V_rv = cmd_velocity;
			...
			float th_err = way_theta - Rev_odom_onewheel_w;
			...
			if(isInitial_move){
				...
				if(time_count_avoid < cnt_limit){
					way_theta_stime =  Rev_odom_onewheel_w + (th_err/cnt_limit)*time_count_avoid;
					time_count_avoid += 1;
        ...
        V_rv = cmd_velocity;
			...
			if(isInitial_move){
				if(fabs(Rev_odom_onewheel_w - W_rw) <= 0.5) {
	...
	void onewheel_vw::joystickCallback(const move_robot::joystick& joystick)
	{
	...
	void onewheel_vw::joystick_move()
	{//==============change=============
	...
	void onewheel_vw::Calculate_odom()
	{
		  //std::cout<<"Rev_V = "<<Rev_odom_onewheel_v<<" "<<" Rev_W =  "<<Rev_odom_onewheel_w<<std::endl;
	...
	void onewheel_vw::RevProcess(double receive_period)
	{
					...
					for(int i=0; i<readByte; i++)
          {
            ...
            //std::cout<<buff[i]<<std::endl;
				...
				if(rev_buf.size() == 11)
				{
					...
					sendreceive.RevProcess_OneWheel_encoder(rev_buf, Rev_odom_onewheel_v, Rev_odom_onewheel_w);
					rev_buf.clear();
				}
	...
	void onewheel_vw::RevProcess_Battery(double receive_period)
	{
		int Receive_Package_Size = 256;
	...
	void onewheel_vw::Caculate_W_rw(float stop_angle, Eigen::Vector3f robot_pos, float &angular_error, float &pre_angular_error, float &cmd_angular_velocity, bool special)
	{
			float angular_kp = tracking_kp;
			float angular_kd = tracking_kd;
			...
			if(!special)
			{
					angular_error = stop_angle - (robot_pos.z()+Rev_odom_onewheel_w);
					if(fabs(angular_error) > M_PI){
						if(angular_error > 0) angular_error = angular_error - 2*M_PI ;
						else                  angular_error = angular_error + 2*M_PI;
					}
