...
struct SUB_MISSONPATH_SUBPOINT
{ //Jeff add
    int start;
    int start_type;
    int end_type;
...
void Move_Robot::IdTypeCallback(const move_robot::Node_recv &command)
{
    ...
    if (node_buf.type == 15 || node_buf.type == 16 || node_buf.type == 17 || node_buf.type == 18)
    {
        ...
        {
            if (count == 0)
                node_buf.use_backtra = std::atoi(cut_par.c_str());
            else if (count == 1)
                node_buf.turn_time = std::atof(cut_par.c_str());
            else if (count == 2)
                node_buf.turn_R = std::atof(cut_par.c_str());
            count++;
        }
    }
    else
    ...
    if (node_buf.type == 20)
    {
        int count = 0;
        std::string cut_par;
        std::stringstream cut(command.turn_angle);
        while (getline(cut, cut_par, '#'))
        {
            if (count == 0)
                node_buf.use_backtra = std::atoi(cut_par.c_str());
            else if (count == 1)
                node_buf.turn_angle = std::atof(cut_par.c_str());
            count++;
        }
    }
    else
...
void Move_Robot::Misson_state(bool isReSet)
{
    ...
    if (!isReSet)
    {
        ...
        case MISSON_turn_angle:
            std::cout << "MISSON_turn_angle" << std::endl;
            for (int i = 0; i < A_misson[now_A_misson].ALL_pathnode.size(); i++)
            {
                if (A_misson[now_A_misson].ALL_pathnode[i].id == id)
                {
                    turn_use_backtra = A_misson[now_A_misson].ALL_pathnode[i].use_backtra;
                    turn_angle = A_misson[now_A_misson].ALL_pathnode[i].turn_angle;
                    break;
                }
            }
            std::cout << "turn_angle = " << turn_angle << std::endl;
            p_state_ = P_STATE_TURN_ANGLE;
            time_delay_limit = 1;
            break;
