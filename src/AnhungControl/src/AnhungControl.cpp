...
Control::Control(char *address, char *SRV_IP, int port)
{
  ...
  SetLocalpar(LocalparPATH);
	...
	if ((sfd_=socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP))==-1){
	...
	si_me_.sin_port = htons(port);
	si_me_.sin_addr.s_addr = htonl(INADDR_ANY);
	if (bind(sfd_, (struct sockaddr *)&si_me_, sizeof(si_me_))==-1){
	...
	if (inet_aton(SRV_IP, &si_other_.sin_addr)==0) {
		ROS_INFO("inet_aton() failed");
		isSocketOpen_ = false;
	}
	...
	if(isSocketOpen_){
		...
		qrcode_Publisher_ = nodeHandle_.advertise<AnhungControl::qrcode>("qrcode",10);//模擬qrcode訊號
		...
		mission_disSubscriber_ = nodeHandle_.subscribe("mission_dis", 5, &Control::mission_disCallback, this);
...
void Control::mission_disCallback(const std_msgs::Float32& msg)
{
	mission_dis_now = msg.data;
		...
		if(n == -1){
			...
			while(!recv_pkg[count].empty())
			{
				...
				while(getline(cut,cut_par,','))
				{
					...
					else if(recv_pkg_subitem[0]=="Qr"&&count==0)
					{
						count1++;
						re_rqcode = true;
					}
							...
							switch (count1){
							  ...
							  case 8:
								...
								default:
									...
									else if(recv_pkg_subitem[1]=="20") item>>node_buf.turn_angle;
									else {
			...
			if(isMisson_pixel)
			{
				for(int i=0;i<NodeSet.size();i++)
				{
					...
					msg.turn_angle = NodeSet[i].turn_angle;
			...
			if(re_rqcode)
			{
				  std::cout<<"simulation qrcode"<<std::endl;
					AnhungControl::qrcode msg;
					msg.Tag = qrcode_id;
...
void Control::slamoutCallback(const geometry_msgs::PoseStamped& msg)
{
	...
	// Eigen::Vector3f robot_pose_p;
	// robot_pose_p.x() = fabs(robot_pose_.x() - 0.0) / m_resolution;
	// robot_pose_p.y() = 4096 - fabs(robot_pose_.y() - 0.0) / m_resolution;
	...
	if(count==20)
	{
		...
		float all_dis_now = all_dis + mission_dis_now;
		...
		std::string send_all_dis_now = float2str(all_dis_now);
		std::string send_mission_dis_now = float2str(mission_dis_now);
		...
		+ send_all_dis_now + "," + send_mission_dis_now
		+ tail ;
